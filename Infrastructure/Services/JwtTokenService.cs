...
namespace DevQuarter.Wallet.Infrastructure.Services
{
    public class JwtTokenService : ITokenService
    {
        ...
        public async Task<(Result Result, IUser User)> RefreshTokenAsync(string token, string refreshToken, CancellationToken cancellationToken)
        {
            ...
            {
                return (Result.Failure(new[] { "This token has not expired yet." }), null);
            }

            var jti = validatedToken.FindFirstValue(JwtRegisteredClaimNames.Jti);
            var storedRefreshToken = await _walletContext.RefreshTokens.FindAsync(new object[] { refreshToken }, cancellationToken);
            ...
            {
                return (Result.Failure(new[] { "This refresh token does not match this JWT token." }), null);
            }
